...
1. Unary is not a positional numeral system given that the value of a digit does not depend on its position, for instance "1<sub>1</sub>1<sub>2</sub>1<sub>3</sub>" does not differ from "1<sub>3</sub>1<sub>1</sub>1<sub>2</sub>" (Subscripts here are used to identify the position of each digit), or in other words, the variables' concatenations *abc* and *cab* where *a*, *b* and *c* are all equal, are also the same.
